...
for input_biblio in $input_biblios; do
    ...
    for ref_no_ext in "${expanded_refs_no_ext[@]}"; do
        ...
        ' \
        | awk -v ref="$ref_no_ext" '{print $0"\t"ref}' >> "$temp_bed"
    done
...
def get_color_for_ref(ref):
    sorted_refs = sorted(all_refs, key=natural_sort_key)
    if ref in sorted_refs[:15]:
        return manual_colors[sorted_refs.index(ref)]
    else:
