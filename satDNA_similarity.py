...
def best_circular_identity(A: str, B: str) -> float:
    id_fwd = semiglobal_align_identity(A, B + B)
    rcB = revcomp(B)
    id_rc = semiglobal_align_identity(A, rcB + rcB)
...
def run(fasta: str, identity_threshold: float) -> None:
    ...
    for i in range(n):
        ...
        for j, shared in candidates:
            ...
            if best_circular_identity(seqs[i], seqs[j]) >= identity_threshold:
                if uf.union(i, j):
                    unions += 1
    ...
    with open(out_tsv, "w", encoding="utf-8") as tsv:
        ...
        for root, members in sorted(families.items(), key=lambda x: len(x[1]), reverse=True):
            ...
            for m in sorted(members, key=lambda x: ids[x]):
                tsv.write(f"{fam_id}\t{len(members)}\t{ids[rep]}\t{ids[m]}\t{len(seqs[m])}\n")
